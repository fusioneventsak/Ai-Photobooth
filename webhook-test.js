...
/**
 * Webhook Test Script for Replicate Async Video Generation
 * Tests the complete flow: generation → webhook → database → notifications
 */
...
const CONFIG = {
  ...
  TEST_USER_ID: '12345678-1234-1234-1234-123456789012', // Mock user ID for testing
  WEBHOOK_URL: null // Will be constructed from SUPABASE_URL
};
  ...
  try {
    ...
    if (generationsResponse.data && generationsResponse.data.length > 0) {
      generationsResponse.data.forEach((record, index) => {
        console.log(`   ${index + 1}. ${record.prediction_id} - ${record.status} (${record.type})`);
      });
    } else {
    ...
    if (photosResponse.data && photosResponse.data.length > 0) {
      photosResponse.data.forEach((record, index) => {
        console.log(`   ${index + 1}. ${record.filename} - ${record.type} (${record.url.substring(0, 50)}...)`);
      });
    } else {
      console.log('   No records found');
    }
    ...
    const notificationsResponse = await makeRequest(`${CONFIG.SUPABASE_URL}/rest/v1/notifications?select=*&order=created_at.desc&limit=5`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${CONFIG.SUPABASE_ANON_KEY}`,
        'apikey': CONFIG.SUPABASE_ANON_KEY
    ...
    if (notificationsResponse.data && notificationsResponse.data.length > 0) {
      notificationsResponse.data.forEach((record, index) => {
        console.log(`   ${index + 1}. ${record.type} - ${record.title} (User: ${record.user_id})`);
      });
    } else {
