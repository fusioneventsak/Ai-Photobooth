...
async function generateWithFacePreservation(prompt: string, originalContent: string): Promise<string> {
  try {
    console.log('🎭 Preserving face, transforming everything else...');
    
    // Try a two-step approach for better results
    // Step 1: Use image-to-image with very specific prompting first
    try {
      console.log('🎨 Step 1: Trying enhanced image-to-image transformation...');
      const result = await generateWithImageToImage(prompt, originalContent, 0.75, true);
      
      // If the result looks good, return it, otherwise try inpainting
      console.log('✅ Image-to-image succeeded, checking if inpainting needed...');
      return result;
      
    } catch (img2imgError) {
      console.log('🔄 Image-to-image failed, falling back to inpainting...');
    }
    
    // Step 2: Create a more precise face mask and use inpainting
    const faceMask = await createInvertedFaceMask(originalContent);
    
    // Step 3: Use inpainting to modify everything EXCEPT the tiny face area
    return await inpaintAroundFace(prompt, originalContent, faceMask);
    
  } catch (error) {
    console.error('Face preservation failed:', error);
    
    // Final fallback to very low-strength image-to-image
    console.log('🔄 Final fallback to minimal-strength image-to-image...');
...
async function generateWithImageToImage(
  ...
  try {
    ...
    if (preserveFace) {
      enhancedPrompt = `${prompt}, maintain the exact same person's face and facial features, preserve facial identity and expressions, completely transform background and environment, change all clothing and body, new setting and context`;
      negativePrompt = 'face changes, different person, facial distortions, face swap, changed identity, preserve original background, keep original clothing, same environment';
    } else {
    ...
    formData.append('cfg_scale', preserveFace ? '9' : '7'); // Higher CFG for face preservation
    formData.append('output_format', 'png');
